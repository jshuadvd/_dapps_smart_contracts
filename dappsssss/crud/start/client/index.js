...
const initApp = () => {
  ...
  $create.addEventListener('submit', e => {
    ...
    .catch(() => {
      $createResult.innerHTML = 'Opps... There was an error while trying to create a new user...';
    });
  });
  ...
  $read.addEventListener('submit', e => {
    ...
    .catch(() => {
      $readResult.innerHTML = `Opps... There was an error while trying to read the user ${id}`;
    });
  });
  ...
  $edit.addEventListener('submit', e => {
    ...
    .send({from: accounts[0]})
    .then(() => {
      $editResult.innerHTML = `Updated name of user ${id} to ${name}`;
    })
    .catch(() => {
      $editResult.innerHTML = `Opps... There was an error while trying to update user ${id}`;
    });
  });
